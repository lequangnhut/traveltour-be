...
public interface TransportationSchedulesRepository extends JpaRepository<TransportationSchedules, Integer> {
    ...
    @Query(value = "SELECT ts.* " +
            ...
            "WHERE (:transportationBrandId IS NULL OR tb.id = :transportationBrandId) " +
            "  AND (:fromLocation IS NULL OR ts.from_location = :fromLocation) " +
            "  AND (:toLocation IS NULL OR ts.to_location = :toLocation) " +
            "  AND (:departureTime IS NULL OR ts.departure_time >= :departureTime) " +
            "  AND (:arrivalTime IS NULL OR ts.arrival_time <= :arrivalTime) " +
            "  AND (:amountSeat IS NULL OR t.amount_seat >= :amountSeat) " +
    ...
    Page<TransportationSchedules> findTransportationSchedulesWithFilter(
            @Param("transportationBrandId") String transportationBrandId,
